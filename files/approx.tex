...
and $f_n\to f$, and we have pointwise convergences
...
Choose a point $x\in \Sigma$ with defined tangent plane $H_x$.
Note that for any $\eps>0$ there is $\delta>0$ such that if $|x_n-x|<\delta$ for large $n$ and $x_n\in \Sigma_n$, then 
\[|\Zc_{\Sigma_n}(x_n)-\Zc_{\Sigma}(x)|<\eps.\]
Since $M$ is smooth, $\Zc_{\Sigma_n}$ is bounded in $B(p_n,r)_{\Sigma_n}$.
Since $\vol_{m-1}$ on $\Sigma_n$ weakly converges to $\vol_{m-1}$ on $\Sigma$ (see \cite[10.8]{BGP}) we get that 
\[\int_{x\in B(p_n,r)_{\Sigma_n}}\Zc_{\Sigma_n}(x)\cdot \vol^{m-1}\to \int_{x\in B(p,r)}\Zc_{\Sigma}(x)\cdot \vol^{m-1}\]
as $n\to\infty$,
...
we get that $\mathfrak m$ vanisehes in a neighborhood $U\ni p$.
Indeed, we can include in the sequence $M_n$ a subsequence of flat metrics and argue as in the introduction.
...
\[K(\nabla f_n\wedge\nabla h_n)(x)\le K(\sigma)+\delta\cdot|\Rm(x)|\]
for all large $n$.
