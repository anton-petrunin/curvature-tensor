...
By Lemma~\ref{lem:test-delta}, the sequence of metric tensors $g_n$ of $M_n$ on $\Omega$ is uniformly delta-converging.
...
where the coefficients $\alpha_{k,n}$ depend continuously on $w_1,\zz\dots, w_{(m-2)\cdot(m-3)/2}$, and the components of $g_n$. 
