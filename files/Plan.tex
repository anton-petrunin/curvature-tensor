...
\parit{(\ref{lem:test-delta-g}).} This part follows from the proof of (\ref{lem:test-delta-partial}) since $g^{ij}_n\zz=\langle d_px_j,d_px_k\rangle_n$ and $g_{ij,n}$ can be expressed thru~$g^{ij}_n$.
...
\parit{Proof of \ref{prop:3parts}(\ref{prop:3parts:reg}) modulo \ref{lem:test-delta}, \ref{lem:test-delta-partial-g}, and \ref{A^0}.}
