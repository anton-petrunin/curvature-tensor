...
The test functions are liftable;
in other words any test function on Alexandrov space can be approximated by a test function on a nearby Riemannian manifold.
This is the main reason to use test functions as the main class.
On the other hand, one test function might be have very different presentations that lead to different test convergences.
Thus to prove invariance of a functional defined on test functions we need to use the $C^1$-delta convergence which is more general.
...
Of cause we have to guarantee the the choosen subclass is liftable.
Hope more natural settings will be found eventually.
